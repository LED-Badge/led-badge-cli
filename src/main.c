...
static arg_list_t argdef[] = {
	...
	{ { 's', "speed"  }, ARG_INT   , NULL       , "start new message at given x coordinate" },
...
static ledbadge_t *ledbadge;
static const ledbadge_info_t *info;
		...
		void *user)
{
	...
	if(id == ARG_ENABLE) {
		if(ledbadge_setup_message(ledbadge, cur_msg, cur_bchar, value.i, cur_mode, cur_speed, cur_options) < 0) {
		...
		char **argv)
{
	...
	if(ledbadge_init() < 0) {
		printf("error initializing ledbadge library\n");
		goto error_0;
	}
	ledbadge = ledbadge_open(NULL);
	if(!ledbadge) {
		printf("error opening ledbadge device\n");
		goto error_1;
	}
	info = ledbadge_info(ledbadge);
	...
	if(arg_list.b) {
		...
		ledbadge_setup(ledbadge, arg_bright.i);
		if(width % info->char_width) {
			printf("invalid width of .png file: must be a multiple of %d for ledbadge\n", info->char_width);
			free_png();
			goto error_2;
		}
		else if(height != info->height) {
			printf("invalid height of .png file: must be %d for ledbadge\n", info->height);
			free_png();
			goto error_2;
		}
		...
		for(y = 0; y < height; y++)
			for(x = 0; x < width; x++)
				if(rows[y][x / 8] & (0x80 >> (x % 8)))
					ledbadge_set(ledbadge, x, y);
		if(!cur_msg && ledbadge_setup_message(ledbadge, 0, 0, width / info->char_width, cur_mode, cur_speed, cur_options) < 0) {
		...
		if(ledbadge_flash(ledbadge) < 0) {
			printf("error flashing ledbadge\n");
			goto error_2;
		}
	}
